...
ebno_dB = np.array([18]);                          #EbN0 values at which Symbols will be created

#BATCHSIZE_MIN = 200000
#BATCHSIZE_MAX = 200000
BATCHSIZE_MIN = 180000
BATCHSIZE_MAX = 180000
...
#Channel from paper
#BAUDRATE    = np.array([100*10**9])
#D           = -5
#L           = np.array([5000])
#wavelen     = np.array([1270])
#beta        = 0.2
#BIAS = 2.25
...
model = Equalizer.SNN_Georg_et_al_Equalizer(10 * EQ_TAP_CNT,
                                            HIDDEN_SIZE,
                                            2**modulation_order,
                                            device=device, 
                                            dt = 1 * 10**(-3) );
